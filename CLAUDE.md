...
- **`subtitle_background`** (`config.yaml` / `FAKEIPTV_SUBTITLE_BACKGROUND`): `false` (default) = injects `::cue { background-color: transparent }` STYLE block → CSS rendering path → transparent background, Televizo outline toggle suppressed (ExoPlayer WebVTT CSS does NOT support `text-shadow`). `true` = no STYLE block → native `CaptionStyleCompat` rendering (black background box, player outline/edge options work). Mutually exclusive in ExoPlayer: any STYLE block forces CSS rendering path which ignores `EDGE_TYPE_OUTLINE`.
